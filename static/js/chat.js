...
function appendMessage(sender, message, data = null) {
    ...
    if (sender.toLowerCase() === 'assistant') {
        ...
        if (data && data.source) {
            ...
            sourceElement.textContent = `Source: ${data.source === 'LLM' ? 'Generated by LLM' : 'From previous response'}`;
...
function setupAutocomplete() {
    ...
    function fetchSuggestions(query) {
        // Example function to fetch suggestions
        return fetch(`/autocomplete?q=${query}`)
