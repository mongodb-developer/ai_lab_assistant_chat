import os
from gevent.pywsgi import WSGIServer
from app import create_app

app = create_app()

if __name__ == '__main__':
    ssl_context = None
    if os.getenv('FLASK_ENV') == 'development':
        cert_path = '/Users/michael.lynn/code/mongodb/ai/ai_lab_assistant_chat/certs/lab-assistant.localhost.com.pem'
        key_path = '/Users/michael.lynn/code/mongodb/ai/ai_lab_assistant_chat/certs/lab-assistant.localhost.com-key.pem'
        
        if os.path.exists(cert_path) and os.path.exists(key_path):
            ssl_context = (cert_path, key_path)
        else:
            print("Warning: SSL certificate files not found. Falling back to non-SSL mode.")

    port = 443 if ssl_context else 8080

    if ssl_context:
        server = WSGIServer(('0.0.0.0', port), app, 
                            keyfile=ssl_context[1], 
                            certfile=ssl_context[0])
        print(f"Starting server in SSL mode on https://0.0.0.0:{port}")
    else:
        server = WSGIServer(('0.0.0.0', port), app)
        print(f"Starting server in non-SSL mode on http://0.0.0.0:{port}")

    if os.getenv('FLASK_ENV') == 'development':
        print("Running in debug mode")
        app.debug = True

    server.serve_forever()